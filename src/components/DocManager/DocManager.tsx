  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] px-40 py-11">
        ...
        {projects.map((project) => (
          <div key={project.id} className="bg-white rounded-2xl border border-[#E5E5E5] p-6 w-[351.66px] h-[243px] flex flex-col">
            ...
            <div className="flex items-center justify-between pt-3">
              ...
              <button onClick={() => navigate(`/doc-manager/${project.id}`)} className="text-[#6B4458] font-normal text-[13px] hover:underline flex items-center gap-1">
                View details
      ...
      <div className="flex items-center justify-end gap-3 pr-2 mt-12">
        <button className="w-[40px] h-[40px] bg-[#E8E8E8] rounded-full flex items-center justify-center hover:bg-[#D8D8D8] transition-colors">
          <ChevronLeft size={18} className="text-[#6B7C93]" />
        </button>
        <button className="w-[40px] h-[40px] bg-[#6B4458] text-white rounded-full flex items-center justify-center font-medium text-[14px]">1</button>
        <button className="w-[40px] h-[40px] bg-[#E8E8E8] rounded-full flex items-center justify-center hover:bg-[#D8D8D8] transition-colors">
