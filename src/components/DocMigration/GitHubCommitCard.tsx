...
export default function GitHubCommitCard({ selectedProject, selectedBranch, onProjectChange, onBranchChange, onCommit }: GitHubCommitCardProps) {
  return (
    <div className="w-full max-w-[600px] mx-auto bg-white rounded-2xl border border-[rgba(226,232,240,0.6)] shadow-[0px_1px_3px_rgba(0,0,0,0.1)] p-5 sm:p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="text-xs bg-[#FFF0F0] text-[#E11D48] px-3 py-1 rounded-full min-h-[32px]">
            Create new branch
          </button>
        </div>
        <select
      ...
      <div className="flex justify-end gap-3">
        <button className="px-6 py-2 border border-[#D1D5DB] rounded-lg text-[#314158] bg-white min-h-[44px]">
          Sync
        </button>
        <button
          onClick={onCommit}
